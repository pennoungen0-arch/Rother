...
- **Name:** Rother — Competitor Review Monitor (package name `rother`, version `0.4.7`).
...
- **Branch:** `test/m15-1-validation`. **Latest commit:** `13121fe` (feat: Phase 2 — GitHub Actions scraper + Vercel remote data layer). **Tag: `v0.4.6`** (latest pushed tag — Vercel web deployment complete). **Tag: `v0.4.7`** — Phase 2 complete: GitHub Actions scraper workflow + Vercel remote data layer. All 8 productization milestones + Phases A–D discovery-first + v0.3.2 self-monitoring fix + systems hardening + harvest honesty + macOS Phase 1 + web launcher scripts + **Vercel Phase 1 (dashboard live) + Phase 2 (scraper integration)** complete.
