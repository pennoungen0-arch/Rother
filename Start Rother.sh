...
# Don't use `set -e` or `set -u` - we want to handle errors ourselves
set +e
...
if [ -f "$PYTHON_DIR/bin/python3.11" ]; then
    ...
    if [ -z "$PYTHON_CMD" ]; then
        ...
        echo "  (This is a one-time download of ~30MB)"
        echo
        ...
        case "$ARCH" in
            x86_64|amd64)
                PYTHON_URL="https://github.com/astral-sh/python-build-standalone/releases/download/20260901/cpython-3.11.16+20260901-x86_64-unknown-linux-gnu-install_only_stripped.tar.gz"
                ;;
            aarch64|arm64)
                PYTHON_URL="https://github.com/astral-sh/python-build-standalone/releases/download/20260901/cpython-3.11.16+20260901-aarch64-unknown-linux-gnu-install_only_stripped.tar.gz"
                ;;
            *)
        ...
        PYTHON_TAR="$TMPDIR/python-standalone-linux.tar.gz"
        PYTHON_TMP="$TMPDIR/python-standalone-linux-extract"
        ...
        if curl -L -o "$PYTHON_TAR" "$PYTHON_URL" 2>/dev/null; then
            ...
            if [ -f "$PYTHON_TMP/bin/python3.11" ]; then
                ...
                chmod +x "$PYTHON_DIR/bin/python3.11" 2>/dev/null || true
                PYTHON_CMD="$PYTHON_DIR/bin/python3.11"
